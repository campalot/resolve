...
export function randomCounterparties(identities: IdentityRecord[]) {
  const identity1 =
...
export function generateInteractionTitle(
...
export function generatePolicyUpdate(workspaceId: string, identities: IdentityRecord[], formData: any) {
  const randomDate = new Date(
    Date.now() - Math.floor(Math.random() * 10000000000)
  ).toLocaleString();
